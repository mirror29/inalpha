...
<br /><sub><strong>因子库</strong> —— 79 因子（pandas-ta / Alpha101 / qlib + FRED 宏观），按当前有效性 IC 排序。</sub>
</td>
</tr>
</table>
...
<tr>
<td width="50%" valign="top">
<img src="assets/screenshots/zh/paper-1.png" alt="Runner 详情 —— 行情" />
<br /><sub><strong>Runner 详情 —— 行情</strong> —— 点进单个模拟盘 runner：累计盈亏（含百分比）、胜率、决策数、风控拦截数一屏可见，每个买卖点都标注在实时 K 线上。</sub>
</td>
<td width="50%" valign="top">
<img src="assets/screenshots/zh/paper-2.png" alt="Runner 详情 —— 持仓与决策" />
<br /><sub><strong>Runner 详情 —— 持仓与决策</strong> —— 当前持仓、逐根 bar 的决策时间线（方向 · 数量 · 成交 · 平仓盈亏），以及该标的按当前 IC 排序的有效因子。</sub>
</td>
</tr>
</table>
...
- **不止择时，还能横截面选股。** 同一批因子也能*在一篮子标的间*排序——`factor.panel_score` 每期按横截面 Rank IC 给整个池打分（谁当下最强、该轮动进谁），与单标的择时口径正交。含 `rank()` 的 Alpha101 横截面因子在 Panel 上原生计算。
...
| ✅ 已上线 | FRED 宏观因子 — Phase 3 | D-12 | 信用利差（HY / IG OAS）· 曲线短端（10Y–3M）· 实体经济（PPI / 工业产出 / 零售）· 情绪（70 → 79 因子） |
...
| 🔬 探索中 | 财报 point-in-time — 更深覆盖 | 待定 | bars 表 PIT 元数据 + 复权处理，超出当前已上线的发布滞后过滤 |
...
如需运行 Evolver，还要生成 Ed25519 凭据授权密钥对，把 DER base64 分别填入
`EVOLUTION_CREDENTIAL_PRIVATE_KEY_B64` / `EVOLUTION_CREDENTIAL_PUBLIC_KEY_B64`；命令见
[`services/evolver/README.md`](services/evolver/README.md)。缺少密钥时演化会 fail closed。
当前演化只接受 DeepSeek、OpenAI、Kimi、智谱各自已有冻结计价的默认模型；普通对话仍可使用
其他模型或自定义代理。
...
无需单独配置控制台 —— 它直接读仓库根的 `.env`（后端 URL + `JWT_SECRET` 都继承），只要第 4 步的
...
> 通过绑定 owner/operation/config/digest 的短效 grant 临时解析加密凭据；首次响应丢失时，
> 同 scope 仅可在两分钟内补偿重试一次，
> 成功后从队列清除 grant，
> run 内只保留冻结的非敏感配置/计价快照。想用多个独立 terminal 手动起，或看底层 live trace（`mastra dev`
