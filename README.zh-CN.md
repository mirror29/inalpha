...
> **当前状态：** Inalpha 处于 **alpha** 阶段（Phase D-11，多市场模拟盘：跨币种 cash + live runner 让 promoted 策略按行情自动跑；建立在 D-10 多市场数据、D-9 LLM 自创策略 + 风控引擎之上）。欢迎读代码、参与设计——**请勿用真实资金跑**（真钱实盘不在当前计划）。
...
| **统一内核** | 回测、模拟盘共用一份策略代码——切换的是 Clock 与 Gateway，不是逻辑。内核同代码设计（延伸到实盘只是换个 Gateway），但真钱实盘不在当前计划。行为分歧时根因只剩物理差异（滑点、延迟、数据精度），而非"两条代码路径" |
...
<p align="center">
  <img src="assets/architecture.png" alt="Inalpha 系统架构" width="820" />
</p>
...
- **L3 · Python kernel services（FastAPI）。** 每个 service 是独立的有状态进程。已落地：`services/data`（行情接入 + web 搜索 + 财报基本面，覆盖 A股/港股/美股/全球）、`services/paper`（事件驱动内核，回测 = 模拟盘 同代码）、`services/research`（多 agent 起手脚手架；analyst 接基本面 + web 情报兜底；完整 bull/bear 辩论闭环放在 Phase E+）、`services/factor`（因子库 pandas-ta / Alpha101 / qlib + IC 有效性检验，只产出信号、不下单）。`Strategy Evolution` 异步循环并行运行。
...
两张图由 D2 源文件渲染——[`assets/architecture.d2`](assets/architecture.d2) / [`assets/strategy-evolution.d2`](assets/strategy-evolution.d2)。当前已落地的模块清单、未完成项、决策链路 sequence diagram 见 [`docs/04-current-state.md`](docs/04-current-state.md)。
...
| 🗓️ 已规划 | 策略进化 — E2 | D-12 | 多代演化 + MAP-Elites + Island Model + `unified-diff` 变异 |
...
**操作者控制台**是使用 Inalpha 的推荐方式。它是一块只读运行时看板，把原本要问 agent 才知道的
...
> 控制台正在成为唯一的统一入口：**和 agent 对话的功能后续也会搬进控制台**，最终你将在同一个
> 地方驱动数据、研究、回测、Live Runner，以及与 orchestrator 的对话。

### 5 · 跟 orchestrator 对话

在对话搬进控制台之前，先打开 `mastra dev` playground **<http://127.0.0.1:4111>** —— 在这里跟
orchestrator agent 聊天，并在 live trace UI 里看每一次 tool 调用、hook 事件、approval token 流转。
`services/paper` 不直接调 LLM；只有 orchestrator（Mastra）和 `services/research` 会消耗你的 key。
