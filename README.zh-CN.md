...
> **当前状态：** Inalpha 处于 **alpha** 阶段（Phase D-11，多市场模拟盘：跨币种 cash + live runner 让 promoted 策略按行情自动跑；建立在 D-10 多市场数据、D-9 LLM 自创策略 + 风控引擎之上）。欢迎读代码、参与设计——**暂不建议用真实资金跑。**
...
- **L3 · Python kernel services（FastAPI）。** 每个 service 是独立的有状态进程。已落地：`services/data`（行情接入 + web 搜索 + 财报基本面，覆盖 A股/港股/美股/全球）、`services/paper`（事件驱动内核，回测 = 模拟盘 = 实盘同代码）、`services/research`（多 agent 起手脚手架；analyst 接基本面 + web 情报兜底；完整 bull/bear 辩论闭环放在 Phase E+）。`Strategy Evolution` 异步循环并行运行。
