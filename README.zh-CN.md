...
- **L3 · Python kernel services（FastAPI）。** 每个 service 是独立的有状态进程。已落地：`services/data`（行情接入 + web 搜索 + 财报基本面，覆盖 A股/港股/美股/全球）、`services/paper`（事件驱动内核，回测与模拟盘共用同一内核）、`services/research`（多 agent 起手脚手架；analyst 接基本面 + web 情报兜底；完整 bull/bear 辩论闭环放在 Phase E+）、`services/factor`（因子库 pandas-ta / Alpha101 / qlib + IC 有效性检验 + 当前有效因子择时，只产出信号、不下单）。`Strategy Evolution` 异步循环并行运行。
