...
> **当前状态：** Inalpha 处于 **alpha** 阶段（Phase D-10，LLM 自创策略 + 风控引擎规则化 + 多市场数据：web 搜索 + 财报基本面 + 全球品种覆盖）。欢迎读代码、参与设计——**暂不建议用真实资金跑。**
...
| ⏭️ 进行中 | 多市场模拟盘 — Live runner + 跨币种 cash | D-11（issue #1） | 行情 tick 驱动 `on_bar`、写 `paper_positions` / `paper_trades` · 按币种 cash 桶 + FX 折算 equity |
