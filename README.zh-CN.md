...
> **S1 实现**：单机 ProcessPoolExecutor + Mastra `foreach({concurrency:4})`，单 tool `swarm.run_backtest_grid`，grid 上限 20 由 PreToolUse hook 强制。Smoke：`pnpm tsx packages/orchestration/scripts/smoke-swarm.ts`。设计细节 + graduation 标准见内部 ADR-0025。
