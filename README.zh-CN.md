...
<p>有效因子择时 &nbsp;·&nbsp; 多视角研究 &nbsp;·&nbsp; 因子实验室 &nbsp;·&nbsp; 风控引擎 &nbsp;·&nbsp; 策略进化 &nbsp;·&nbsp; 机器审批下单 &nbsp;·&nbsp; 回测=模拟盘同代码 &nbsp;·&nbsp; 狐神签</p>
...
- **狐神签（趣味彩蛋）。** 方向犹豫时求一卦 / 抽张塔罗，给个数据之外的参照视角；**硬隔离于决策**，风控 / 下单 / 因子都碰不到（详见下方核心能力 §6）。
...
- **L1 · 用户入口。** 操作者控制台（`apps/dashboard`，内嵌 agent 对话栏）是主入口；也可用 `mastra dev` playground 看 live trace，或直接调 tool CLI。
...
- **L3 · Python kernel services（FastAPI）。** 每个 service 是独立的有状态进程。已落地：`services/data`（行情接入 + web 搜索 + 财报基本面，覆盖 A股/港股/美股/全球）、`services/paper`（事件驱动内核，回测 = 模拟盘 同代码）、`services/research`（多 agent 起手脚手架；analyst 接基本面 + web 情报兜底；完整 bull/bear 辩论闭环放在 Phase E+）、`services/factor`（因子库 pandas-ta / Alpha101 / qlib + IC 有效性检验 + 当前有效因子择时，只产出信号、不下单）。`Strategy Evolution` 异步循环并行运行。
...
- **完整的 bull/bear 结构化辩论**在 Phase E+ 收口；当前已能多视角同台 + 对立观点喂进综合。
