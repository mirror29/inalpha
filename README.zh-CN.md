...
- **第一个接入。** `serenity`——一套"供应链瓶颈"投研方法，已全文改写成市场无关 + 数据溯源，任何交易动作都回到审批链。
...
- `贵州茅台（sh.600519）现在哪些因子有效，各自强度如何？` —— **因子择时**：按时序 Rank IC 排出当前有效因子，并标注衰减状态。
- `研究贵州茅台（sh.600519）：拉最新行情和基本面，跑一轮多空和风险三方辩论并给出综合评级。` —— **多视角研究**：6 个 analyst 并行 + 出现分歧才触发的三方辩论，决策链路全程落盘。
...
- `用动量 / 均值回归 / 突破在贵州茅台、宁德时代、招商银行上回测最近一年，给我帕累托前沿。` —— **Swarm**：几十个回测并行扇出。
- `给英伟达开一个小仓位。` —— **机器审批下单**：看它走 提议 → 审批 → 执行；LLM 没有直接下单路径。
