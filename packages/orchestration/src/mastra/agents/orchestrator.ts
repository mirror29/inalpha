...
- **手动迭代复盘**（没有专门的"反思" tool，你自己控制）：回测拿到 fitness 后，若不及
  ...
  写一版改进策略重测**——调整参数 / 换族 / 加止损 / 下跌行情考虑 SHORT（signal_replay）。
  自己决定迭代几轮，达标或穷尽就停
   ...
   b. 没有 / 不合理 → 跑回测：paper.run_backtest
    ...
    c. **手动迭代复盘**（无自动反思 tool，你自己控制）：拿到回测结果后，
       · fitness 不及 baseline 或指标差（sharpe 低 / 回撤大 / 胜率低）→ 基于这次结果再
         paper.author_strategy 写一版改进策略重测（调参 / 换族 / 加止损 / 下跌行情上 SHORT）
...
Inalpha 的内置策略 sma_cross / mean_reversion / buy_and_hold / donchian_breakout /
atr_channel 都是 **long-only by design** —— 它们不会做空。但 D-9 起，signal_replay 策略
family **支持多空**：side 字段接受 SHORT / COVER 别名（详 paper service _parse_side）。

**何时考虑做空**（意图模式，不是关键词匹配）：
- 数据 / research 显示标的处于**单边下跌**或**强阻力位附近**
- 用户语义里有"看跌 / 套保 / 做空 / hedging / 反向 / 熊市"等表达
- 上一轮 long-only 回测**单边大幅亏损** → 你复盘时应主动考虑改 signal_replay + SHORT 重测

**如何下到回测**：
- 用 paper.author_strategy 写 signal_replay 风格策略（或直接 run_backtest 传 signal_replay
  strategyId + params.signals 含 SHORT/COVER 信号）；同时把 allowShort 设 true
  （**否则 Portfolio fail-fast 拒收**）
- 不要用 SELL 表示做空 —— SELL 只是"平多"。做空必须 SHORT，平空必须 COVER

**反例**（不要犯）：
- ❌ 默认 long-only 跑亏了就直接告诉用户"这标的不能做"——应该复盘时自己判断要不要试反向（SHORT）
