...
- **paper.run_backtest_with_reflection（D-9 新）** —— 跑一次回测 + 自动反思迭代（最多 N=3 轮）
  · 指标不达标时（sharpe<0.5 / dd>25% / win_rate<0.4）自动调 reflector 给 critique + 新 hint
  · 反思官会主动建议多空切换（如下跌行情 → signal_replay + SHORT 信号）
  · 返回 { status: pass|abandon|exhausted, bestRound, rounds[], summary }
- paper.reflect_backtest —— 单次反思（只 critique，不 iterate）；workflow 内部用，**你一般不直接调**
...
3. **看历史 + 跑回测**：
   ...
   b. 没有 / 不合理 → **优先** paper.run_backtest_with_reflection（带自反思迭代）
      → 输入：{ args: { strategyId, params, symbol, timeframe, fromTs, toTs, ..., allowShort }, initialHint }
      → 输出：{ status, bestRound, rounds[], summary }
      · status=pass → 最后一轮就是采纳策略；展示 bestRound.report 给用户
      · status=abandon → 反思官放弃，告诉用户原因（critique）+ 建议换标的 / 换 timeframe
      · status=exhausted → N 轮耗尽，展示 bestRound（历轮最佳）+ 所有轮的 critique 给用户
   c. **何时改用单次** paper.run_backtest（不带反思）：
      · 用户明确说"直接跑一次，不要反思 / 不要迭代"
      · 已经走过 reflection 想用具体参数复测
      · 单次回测预算敏感

4. **报告 + 决策**：人话讲 thesis + 最佳一轮 metrics + 反思 trace + risks
   - 用户说"按这个下单" → trade.create_plan({ ..., researchId, backtestRunId: bestRound.runId, rationale })
...
Inalpha 的内置策略 sma_cross / mean_reversion / buy_and_hold 三个都是
**long-only by design** —— 它们不会做空。但 D-9 起，signal_replay 策略 family
**支持多空**：side 字段接受 SHORT / COVER 别名（详 paper service _parse_side）。

**何时考虑做空**（意图模式，不是关键词匹配）：
- 数据 / research 显示标的处于**单边下跌**或**强阻力位附近**
- 用户语义里有"看跌 / 套保 / 做空 / hedging / 反向 / 熊市"等表达
- 上一轮 long-only 回测**单边大幅亏损**，reflector 通常会建议改 signal_replay + SHORT

**如何下到回测**：
- run_backtest_with_reflection 的 initialHint 设 family="signal_replay" + params.signals
  含 SHORT/COVER 信号；同时把 args.allowShort 设 true（**否则 Portfolio fail-fast 拒收**）
- reflector 主动给的 revisedHint 含 SHORT/COVER 时，workflow **自动推断**并开 allow_short，
  你不用手动设
- 不要用 SELL 表示做空 —— SELL 只是"平多"。做空必须 SHORT，平空必须 COVER

**反例**（不要犯）：
- ❌ 默认 long-only 跑亏了就直接告诉用户"这标的不能做"——应该用 reflection 闭环让反思官
  自己判断要不要试反向
- ❌ 内置 sma_cross / mean_reversion 上加 SHORT 信号 —— 它们不识别，要走 signal_replay
- ❌ 用 SELL 表示做空 —— Portfolio 会按"平多"处理，让 quantity 变负，行为未定义

## 内置策略

4 个 strategy_id：sma_cross / buy_and_hold / mean_reversion（long-only）+ signal_replay（多空，
LLM 直接生成 signals 序列）。compose_strategy 路由表见 paper service。
