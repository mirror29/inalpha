...
2. **设计策略（D-9 默认路径 = author）**：
   ...
   - **因子血缘必传**（ADR-0047）：把 deep_dive factors / factor.timing 的 top 因子
     （含 rank_ic / rank_ic_recent / decay_state）原样填进 factorContext——promoted 上
     模拟盘后系统按它巡检衰减并在活动流告警。数值必须来自真实工具返回，禁止编造；
     decaying 的因子不要当核心信号
