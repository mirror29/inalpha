...
1. **研究**：research.deep_dive({ symbol, timeframe, asOf: <现在>, lookbackDays: <按市场> })
   ...
   - **投资大师视角（personas，可选；默认不传）**：当用户意图是"想看不同投资风格/
     ...
     ⚠️ 普通研究意图（只问行情 / 买卖点 / 走势，不涉及风格对比）**不要**带 personas —— 省 token。
     "几位大师 / 多空对立视角看看" → 自行挑 2-4 个**低相关**的（如 buffett + wood + marks）。
     ⚠️ 普通研究意图（"X 能买吗" / "看下走势" / "什么观点"）**不要**带 personas —— 省 token。
...
3. **看历史 + 跑回测**：
   ...
   b. 没有 / 不合理 → 二选一：
      · **候选策略路径（LLM 自创）**：paper.run_backtest({ candidateId, symbol, timeframe, researchId, strategyHint })
      ...
      · **内置策略 + 反思迭代路径**：paper.run_backtest_with_reflection（带自反思迭代）
    ...
    c. **何时改用单次** paper.run_backtest（不带反思 / 不带 candidateId）：
