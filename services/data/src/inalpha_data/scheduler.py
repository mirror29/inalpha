...
async def record_snapshot(db: AsyncConnection, *, index_code: str) -> tuple[str, int]:
    """拉 ``index_code`` 当前成分（akshare）落库 ``as_of_date=今天``，返回 ``(日期, 条数)``。
    ...
    async with db.transaction():
        n = await store.upsert_snapshot(
            db, index_code=index_code, as_of_date=today, constituents=members
        )
        ...
        for index_code in self._index_codes:
            try:
                async with get_conn() as db:
                    ...
                    await record_snapshot(db, index_code=index_code)
            except asyncio.CancelledError:
                raise
